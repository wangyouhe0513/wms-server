...
def personal_bill(
    ...
    total = q.count()
    rows = q.order_by(Transaction.trans_date.desc(), Transaction.created_at.desc(), Transaction.id.desc()).offset((page-1)*page_size).limit(page_size).all()
...
@app.get("/api/finance")
def finance_list(year: int = 0, month: int = 0, person: str = "", category: str = "", type: str = "", page: int = 1, page_size: int = 30, db: Session = Depends(get_db)):
    """财务记录列表"""
